...
def home(request):
    if 'id' in request.session.keys():
        if request.session['type'] == 'customer':
            ...
            return render(request, 'foody/userhome.html', context)
            ...
            return render(request,'foody/resthome.html', context)
    else:
        return render(request, 'foody/login.html')
...
def signup(request):
	if request.method == 'POST':
		...
		usertype = request.POST.get('usert')
		if usertype == 'Customer':
			Customer.objects.create(name=name, email=email, phone=phone, address=address, city='India', password=password)
			user = Customer.objects.get(email=email)
			#user = Customer(name=name, email=email, phone=phone, address=address, city='India')
			#user.set_password(user.make_password(password))
			user.save()
			request.session['id'] = email
			request.session['type'] = 'customer'
		elif usertype == 'Restaurant':
			Restaurant.objects.create(name=name, email=email, phone=phone, address=address, city='India', password=password)
...
def editDetails(request):
	if request.method == 'POST':
		if request.session['type'] == 'customer':
			...
			return render(request, 'foody/userdetails.html',context)
		elif request.session['type'] == 'restaurant':
			restaurant = Restaurant.objects.get(email=request.session['id'])
			context = {
				'restaurant': restaurant,
			...
			restaurant.save()
			messages.success(request,'Successfully saved :)')
			return render(request,'foody/restdetails.html',context)

	elif request.method == 'GET':
		return render(request,'foody/details.html')
...
def changePassword(request):
	if request.method == "POST":
		if request.session['type'] == 'customer':
			...
			return render(request,'foody/changePassword.html')
		elif request.session['type'] == 'restaurant':
			restaurant = Restaurant.objects.get(email=request.session['id'])
			oldPassword = request.POST.get('oldPassword')
			newPassword = request.POST.get('newPassword')
...
def restaurantOrderHistory(request):
	...
	return render(request,'foody/resthistory.html',context)
...
def restprofile(request):
	...
	return render(request, 'foody/restprofile.html', context)
...
def restview(request,restname):
	if 'id' in request.session.keys():
		try:
			...
			return render(request,'foody/restview.html',context)
		except ObjectDoesNotExist:
			return HttpResponse("No Restaurant with this name, thik se pta karke aao")
	else:
		return redirect('/')
		...
		if request.method == 'GET':
			...
			for x in query:
				if x.customer.email == customer.email:
					amount = amount + x.fooditems.price * x.foodqty
					try:
						cart[x.fooditems.resid].append(x)
					except KeyError:
						cart[x.fooditems.resid] = [x]
			...
			return render(request,"foody/ordercart.html",context)
			...
			for q in Cart.objects.all():
				if q.customer == Customer.objects.get(email=request.session['id']):
					try:
						orders[q.fooditems.resid] = orders[q.fooditems.resid] + ',' + str(q.fooditems.pk)
					except KeyError:
						orders[q.fooditems.resid] = str(q.fooditems.pk)
					try:
						ordersqty[q.fooditems.resid] = ordersqty[q.fooditems.resid] + ',' + str(q.foodqty)
					except KeyError:
						ordersqty[q.fooditems.resid] = str(q.foodqty)
					q.delete()
			...
			return render(request, "foody/ordercart.html")
	else:
		return render(request, "foody/login.html")
...
def history(request):
	if 'id' in request.session.keys():
		...
		return render(request,"foody/userhistory.html",context)
	else:
		return render(request,"foody/login.html")
...
def saveToCart(request):
	if 'id' in request.session.keys():
		...
		for x in foodall:
			if 'food' + str(x.pk) in request.POST.keys():
				if int(request.POST['food' + str(x.pk)]) > 0:
					cartitem = Cart(customer=Customer.objects.get(email=request.session['id']), fooditems=FoodItem.objects.get(pk=x.pk), foodqty=request.POST['food' + str(x.pk)])
					cartitem.save()
		customer = Customer.objects.get(email=request.session['id'])
		query = Cart.objects.all()
		cart = {}
		...
		for x,y in cart.items(): # items  ~ iteritems
			for z in y:
				z.fooditems.ordercount = z.fooditems.ordercount + z.foodqty
				z.fooditems.save()
		return render(request, "foody/ordercart.html", context)
	else:
		return render(request, "foody/login.html")
...
def about(request):
	if 'id' in request.session.keys():
		...
		return render(request,"foody/about.html",context)
	else:
		return render(request,"foody/about.html")